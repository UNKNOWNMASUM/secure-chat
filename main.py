...
class User(Base):
    ...
    gender = Column(String, nullable=True)  # "male" | "female" | "other" | "prefer_not"
...
class StatusPost(Base):
    ...
    text_content = Column(String, nullable=True)   # the text itself, or a caption for media
    bg_color = Column(String, nullable=True)        # background color hex, for text-only statuses
    file_url = Column(String, nullable=True)
    file_type = Column(String, nullable=True)
    is_permanent = Column(Boolean, default=False, nullable=False)  # True = saved to profile, never expires
    created_at = Column(DateTime, default=datetime.datetime.utcnow, index=True)
...
class MessageReaction(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    # "dm" -> references Message.id, "group" -> references GroupMessage.id
...
def _ensure_user_columns():
    """Add newer profile columns on existing databases without wiping data."""
    ...
    try:
        ...
        alters = []
        if "bio" not in cols:
            alters.append("ALTER TABLE users ADD COLUMN bio VARCHAR")
        if "gender" not in cols:
            alters.append("ALTER TABLE users ADD COLUMN gender VARCHAR")
        if "date_of_birth" not in cols:
            alters.append("ALTER TABLE users ADD COLUMN date_of_birth VARCHAR")
        if "notifications_enabled" not in cols:
            alters.append("ALTER TABLE users ADD COLUMN notifications_enabled BOOLEAN DEFAULT 1")
        if alters:
...
def read_me(current_user: User = Depends(get_current_user)):
    return {
        ...
        "notifications_enabled": bool(getattr(current_user, "notifications_enabled", True)),
    }
...
def update_profile(
    ...
    if payload.bio is not None:
        current_user.bio = payload.bio.strip()[:200] or None
    if payload.gender is not None:
        g = payload.gender.strip().lower()
        if g and g not in ("male", "female", "other", "prefer_not"):
            raise HTTPException(status_code=400, detail="Invalid gender")
        current_user.gender = g or None
    if payload.date_of_birth is not None:
        dob = payload.date_of_birth.strip()
        current_user.date_of_birth = dob or None
...
def send_message(
    ...
    # Real-time notify recipient (fire-and-forget). Keep this route sync so we
    # don't hit SQLAlchemy MissingGreenlet (error f405) with sync Session.
    preview = payload.text or (f"📎 {payload.file_name}" if payload.file_name else "New message")
    if len(preview) > 120:
        preview = preview[:117] + "..."
    notify_payload = {
        "type": "new-message",
    ...
    to_email = payload.receiver_email
    try:
        import asyncio
        loop = asyncio.get_event_loop()
        if loop.is_running():
            loop.create_task(manager.send_to(to_email, notify_payload))
        else:
            loop.run_until_complete(manager.send_to(to_email, notify_payload))
    except Exception:
        pass
...
def get_conversation(
    ...
    reactions_by_id = _reactions_for("dm", [r.id for r in rows], current_user, db)
    result = []
    for row in rows:
        if row.is_deleted:
            text = None
        ...
        result.append(
            MessageOut(
                ...
                reactions=reactions_by_id.get(row.id, []),
...
def search_users(
    ...
    if not q:
        return []
    # Case-insensitive LIKE works on both SQLite and PostgreSQL
    # (ilike is Postgres-only and fails / no-ops on SQLite)
    like_q = f"%{q.lower()}%"
    users = (
        db.query(User)
        .filter(
            or_(
                ...
                func.lower(func.coalesce(User.phone, "")).like(like_q),
            )
        )
...
def _reactions_for(message_type: str, message_ids: List[int], current_user: User, db: Session) -> Dict[int, List[ReactionOut]]:
    ...
    rows = (
        ...
        .filter(MessageReaction.message_type == message_type, MessageReaction.message_id.in_(message_ids))
        .all()
    )
    by_message: Dict[int, Dict[str, ReactionOut]] = {}
    for r in rows:
        msg_bucket = by_message.setdefault(r.message_id, {})
        if r.emoji not in msg_bucket:
            msg_bucket[r.emoji] = ReactionOut(emoji=r.emoji, count=0, reacted_by_me=False)
        msg_bucket[r.emoji].count += 1
        if r.user_id == current_user.id:
            msg_bucket[r.emoji].reacted_by_me = True
    return {mid: list(bucket.values()) for mid, bucket in by_message.items()}
...
def toggle_reaction(
    ...
    if payload.message_type not in ("dm", "group"):
    ...
    # Confirm the target message exists (and, for groups, that the user is a member)
    if payload.message_type == "dm":
        msg = db.query(Message).filter(Message.id == payload.message_id).first()
        if not msg or msg.is_deleted:
            raise HTTPException(status_code=404, detail="Message not found")
        if current_user.email not in (msg.sender.email, msg.receiver_email):
            raise HTTPException(status_code=403, detail="Not part of this conversation")
    else:
        msg = db.query(GroupMessage).filter(GroupMessage.id == payload.message_id).first()
        if not msg or msg.is_deleted:
            raise HTTPException(status_code=404, detail="Message not found")
        _require_group_member(msg.group_id, current_user, db)

    existing = (
        db.query(MessageReaction)
        .filter(
            MessageReaction.message_type == payload.message_type,
...
def get_group_messages(
    ...
    reactions_by_id = _reactions_for("group", [r.id for r in rows], current_user, db)
    result = []
    for row in rows:
        if row.is_deleted:
            text = None
...
def _status_is_active(post: "StatusPost") -> bool:
    if post.is_permanent:
        return True
    age = datetime.datetime.utcnow() - post.created_at
    return age < datetime.timedelta(hours=STATUS_EXPIRE_HOURS)
...
def create_status(
    ...
    if payload.content_type not in ("text", "image", "video"):
        raise HTTPException(status_code=400, detail="Invalid content_type")
    if payload.content_type == "text" and not payload.text_content:
        raise HTTPException(status_code=400, detail="Text status needs text_content")
    if payload.content_type in ("image", "video") and not payload.file_url:
        raise HTTPException(status_code=400, detail="Media status needs a file")
...
@app.get("/api/status/feed", response_model=List[StatusFeedEntryOut])
def get_status_feed(
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    # Everyone the user has exchanged a DM with, a shared group with, or
    # themselves — this stands in for a "contacts" list since there's no
    # separate contact/friend system in this app.
    dm_partners = set()
    for m in db.query(Message).filter(
        (Message.sender_id == current_user.id) | (Message.receiver_email == current_user.email)
    ).all():
        dm_partners.add(m.sender_id)
        other = db.query(User).filter(User.email == m.receiver_email).first()
        if other:
            dm_partners.add(other.id)
    group_ids = [gm.group_id for gm in db.query(GroupMember).filter(GroupMember.user_id == current_user.id).all()]
    for gm in db.query(GroupMember).filter(GroupMember.group_id.in_(group_ids)).all():
        dm_partners.add(gm.user_id)
    dm_partners.add(current_user.id)

    all_posts = (
        db.query(StatusPost)
        .filter(StatusPost.user_id.in_(dm_partners))
    ...
    for user_id, posts in by_user.items():
        ...
        owner = posts[0].user
        has_unseen = any(p.id not in viewed_ids for p in posts if p.user_id != current_user.id)
        posts_out = [
            StatusPostOut(
                id=p.id,
    ...
    # Own status first, then users with unseen updates, then the rest — newest first within each group
    result.sort(key=lambda e: (e.user_email != current_user.email, not e.has_unseen, -e.posts[-1].created_at.timestamp()))
    return result
...
def view_status(
    ...
    if post.user_id == current_user.id:
        return {"message": "ok"}  # no need to record views of your own status
    existing = (
        db.query(StatusView)
        .filter(StatusView.status_id == status_id, StatusView.viewer_id == current_user.id)
        .first()
    )
    if not existing:
...
@app.get("/api/status/{status_id}/viewers", response_model=List[StatusViewerOut])
def get_status_viewers(
    ...
    views = db.query(StatusView).filter(StatusView.status_id == status_id).order_by(StatusView.viewed_at.desc()).all()
    return [
        StatusViewerOut(
...
async def websocket_endpoint(websocket: WebSocket, token: str = Query(...), db: Session = Depends(get_db)):
    ...
    if user is None:
        await websocket.close(code=4401)
        return
    # Copy scalars before any await — avoids SQLAlchemy MissingGreenlet (f405)
    ...
    try:
        while True:
            ...
            outgoing = {"type": msg_type, "from": email, "from_username": from_username}
...
@app.get("/sw.js")
def service_worker():
    # Must be served from root scope so notifications work app-wide
