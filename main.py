...
# In-memory last-seen timestamps. Online users are tracked by the WebSocket.
# This survives normal reconnects, but resets if the server itself restarts.
last_seen_map: Dict[str, datetime.datetime] = {}
...
def _reaction_summary(model, message_id: int, current_user_id: int, db: Session):
    rows = db.query(model).filter(model.message_id == message_id).all()
    counts: Dict[str, int] = {}
    my_reaction = None
    for r in rows:
        ...
        if r.user_id == current_user_id:
            my_reaction = r.emoji
    summary = [ReactionSummary(emoji=e, count=c) for e, c in counts.items()]
    return summary, my_reaction
...
def react_to_message(
    ...
    summary, my_reaction = _reaction_summary(MessageReaction, message_id, current_user.id, db)
    return {"reactions": summary, "my_reaction": my_reaction}


@app.post("/api/groups/{group_id}/messages/{message_id}/react")
def react_to_group_message(
    ...
    existing = (
        ...
        .filter(GroupMessageReaction.message_id == message_id, GroupMessageReaction.user_id == current_user.id)
        .first()
    )
    if existing and existing.emoji == payload.emoji:
        db.delete(existing)
        ...
        if existing:
            db.delete(existing)
            db.commit()
        db.add(GroupMessageReaction(message_id=message_id, user_id=current_user.id, emoji=payload.emoji))
        db.commit()

    summary, my_reaction = _reaction_summary(GroupMessageReaction, message_id, current_user.id, db)
    return {"reactions": summary, "my_reaction": my_reaction}
...
def get_conversation(
    ...
    for row in rows:
        if row.is_deleted:
            ...
            text = None
        reactions, my_reaction = _reaction_summary(MessageReaction, row.id, current_user.id, db)
        result.append(
            MessageOut(
                id=row.id,
...
def get_group_messages(
    ...
    for row in rows:
        if row.is_deleted:
            ...
            text = None
        reactions, my_reaction = _reaction_summary(GroupMessageReaction, row.id, current_user.id, db)
        result.append(
            GroupMessageOut(
                id=row.id,
...
async def send_group_message(
    ...
    member_emails = [
        email for (email,) in (
            db.query(User.email)
            .join(GroupMember, GroupMember.user_id == User.id)
            .filter(GroupMember.group_id == group_id, User.email != current_user.email)
            .all()
        )
    ]
    notification = {
    ...
    for email in member_emails:
        await manager.send_to(email, notification)
    return {"id": msg.id, "created_at": msg.created_at}
...
def get_users_status(
    ...
    requested.discard(current_user.email)
    result = []
    for email in requested:
        result.append({
            "email": email,
            "online": email in manager.active,
            "last_seen": last_seen_map.get(email),
        })
    return result


# ---------------------------------------------------------------------------
# WebSocket: calls, chat notifications, and presence
# ---------------------------------------------------------------------------
@app.websocket("/ws")
async def websocket_endpoint(websocket: WebSocket, token: str = Query(...), db: Session = Depends(get_db)):
    ...
    last_seen_map.pop(email, None)
    await manager.broadcast({"type": "presence-online", "email": email})
    try:
        while True:
            ...
            allowed_types = {
                "call-offer", "call-answer", "ice-candidate", "call-reject",
                "call-end", "call-busy", "typing-start", "typing-stop"
            }
            if msg_type not in allowed_types or not to_email:
                continue
        ...
        last_seen_map[email] = datetime.datetime.utcnow()
        await manager.broadcast({"type": "presence-offline", "email": email, "last_seen": last_seen_map[email].isoformat()})
    except Exception:
        manager.disconnect(email)
        last_seen_map[email] = datetime.datetime.utcnow()
        await manager.broadcast({"type": "presence-offline", "email": email, "last_seen": last_seen_map[email].isoformat()})
